...
if len(sys.argv) != 2:
    print("ERROR: you must specify the name of genome fasta file or its path as the first argument")
    sys.exit()
...
def chr_start_end(genome_fasta):
    ...
    ## get strain name from fasta files either from a path or in tne src directory
    ## from a path
    if "/" in genome_fasta:
    ...
    for seq_record in SeqIO.parse(genome_fasta, "fasta"):
        ...
        for j in range(revoffset, len(seq_record.seq)-19):
    	    ...
    	    if revmot.count("C") >= 8 and revmot.count("A") >= 3 and revmot.count("C") + revmot.count("A") >= 17 and "AAAA" not in revmot:
    	        revcount += 1
    	    else:
    	    	break
        ...
        with open("telom_length.csv", "a") as filout:
            if file_exists:
                filout.write("{0}\t{1}\tL\t{2}\t{4}\n{0}\t{1}\tR\t{3}\t{5}\n".format(strain, chrom, left_tel, right_tel, offset, revoffset))
            else:
                filout.write("{0}\t{1}\t{2}\t{3}\t{4}\n{5}\t{6}\tL\t{7}\t{9}\n{5}\t{6}\tR\t{8}\t{10}\n".format("Strain", "Chromosome", "Contig_side", "Telom_length", "Offset", strain, chrom, left_tel, right_tel, offset, revoffset)) # file doesn't exist yet, write a header
